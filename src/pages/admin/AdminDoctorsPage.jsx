...
//   try {
//     const res = await api.get(`/doctors/page?page=${page}&size=${pageSize}`);
...
//     try {
//       await api.post('/registerDoctor', null, { params: { username, password, fullName, phone, specializationId } });
//       return true;
//     } catch (err) {
...
const AdminDoctorsPage = () => {
  ...
  const [currentPage, setCurrentPage] = useState(0);
  const [totalPages, setTotalPages] = useState(0);
  const [sortOrder, setSortOrder] = useState('asc'); // ✅ новий стейт
  const pageSize = 10;

  useEffect(() => {
    fetchDoctors(currentPage, sortOrder);
    fetchSpecializations();
  }, [currentPage, sortOrder]); // ✅ додаємо sortOrder до залежностей
  ...
  const handleSortByFullName = () => {
    ...
    setCurrentPage(0); // Повертаємося на першу сторінку
  };
          ...
          <tbody>
            {doctors.map(doc => (
              <tr key={doc.id}>
                ...
                <td>{doc.specialization || '—'}</td>
                <td>
                  <button className="btn btn-sm btn-primary me-2" onClick={() => handleEdit(doc)}>
                    Редагувати
                  </button>
